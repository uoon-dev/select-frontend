...
export class InstantSearch extends React.PureComponent<InstantSearchProps> {
  public render() {
    ...
    if (!isActive || (fetchStatus === FetchStatusFlag.IDLE && !instantSearchList)) {
      return null;
    }
      ...
      <div className="InstantSearchWrapper">
        {fetchStatus === FetchStatusFlag.FETCHING ? null : (
          <ul className="InstantSearchList">
            {instantSearchList
              ? searchType === 'Books'
                ? (instantSearchList as InstantSearchResultBook[]).map((book, idx) => (
                    <li
                      ...
                      >
                        <span
                          ...
                          dangerouslySetInnerHTML={{
                            __html: book.highlight.webTitleTitle
                              ? book.highlight.webTitleTitle
                              : book.title,
                          }}
                        />
                        <span
                          ...
                          dangerouslySetInnerHTML={{
                            __html: getSortedAuthorsHtmlString(
                        ...
                        <span
                          ...
                          dangerouslySetInnerHTML={{
                            __html: book.highlight.publisher
                              ? book.highlight.publisher
                              : book.publisher,
                          }}
                        />
                      </Link>
                ...
                : (instantSearchList as InstantSearchResultArticle[]).map((article, idx) => (
                    ...
                    >
                      <Link
                        className="InstantSearchedName"
                        to={`${articleContentToPath({
                          channelName: article.articleChannel.name,
                          contentIndex: article.contentId,
                        })}?q=${encodeURIComponent(keyword)}&s=instant`}
                      >
                        <span
                          className="InstantSearchTitle"
                          dangerouslySetInnerHTML={{
                            __html: article.highlight.title
                            ...
                            dangerouslySetInnerHTML={{
                              __html: getSortedAuthorsHtmlString(
                                article.highlight.authorNames
                                  ? article.highlight.authorNames
                                  : article.authorsInfo[0].name,
                                article.authorsInfo.length,
                                1,
                              ),
                            }}
                          />
