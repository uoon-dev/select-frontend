...
class MySelect extends React.Component<Props, State> {
  ...
  private areEveryBookChecked = () => {
    ...
    return (
      Object.keys(bookInputs).length > 0 && books.every(book => bookInputs[book.mySelectBookId])
    );
  };
  ...
  public componentDidUpdate(prevProps: Props) {
    ...
    const prevBooksLength =
      ...
      prevProps.mySelectBooks.itemListByPage[prevProps.page].itemList
        ? prevProps.mySelectBooks.itemListByPage[prevProps.page].itemList.length
        : 0;
    if (prevBooksLength !== books.length) {
      // Set up state for checkboxes
      this.setState({
        bookInputs: Object.values(books).reduce(
          (prev, book: MySelectBook) => ({
            ...prev,
                  ...
                  <div className="MySelectBookList_Right">
                    ...
                    >
                      <div className="MySelectBookList_Meta">
                        ...
                        <span className="MySelectBookList_Authors">
          ...
          {!this.isFetched(page) ? (
            ...
            <>
              <Empty
                ...
                description="이전에 이용한 책을 도서 이용 내역에서 확인해보세요."
                iconName="book_1"
              />
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  dispatchLoadMySelectRequest: (page: number) => dispatch(Actions.loadMySelectRequest({ page })),
  dispatchDeleteMySelectRequest: (
